...
**Phases come before epics.** A delivery phase is a commitment, not a dependency: everything in phase *N* is built before anything in phase *N+1*, whether or not the graph requires it — you do not start work under a separate contract because the dependency graph permits it. Specification is deliberately left free to overlap, so an analyst can open the next phase while this one builds. The phase is the epic's `phase` where the source stated one, and derived from `commitment` where it did not (an epic whose stories are all `speculative` is a later phase); every row records which. Each option reports **its first phase's span and standalone cost first**, with the whole-programme figure beside it — a client being quoted an MVP should not be handed a number that silently includes a later contract. Scoring stays on the whole programme, so no option can win by pushing work across the boundary.
...
**Render.** `uv run {skill-root}/scripts/render-plan.py {workspace}/plan.json --workbook {workspace}/estimate.xlsx` writes `plan.md` and **appends** the `Options` tab, a `Gantt — Baseline (1 each)` tab, and one `Gantt — <Archetype>` tab per schedule offered to the workbook est-estimate already produced. The baseline is always there — exactly one person in every role the scope demands, under the recommended archetype — because the best-fit shape of each archetype is always the largest team the sweep allowed, and a reader needs the simplest schedule to calibrate the others against. There is deliberately no second workbook: a client should not have to read a plan beside an estimate to see which stories a bar covers, and every story row links to its own row on the `Stories` tab.

**The Gantt's rows are stories.** Epics are the grouping: an epic band, its stories beneath it, and each story expanding to its `spec` / `build` / `review` / `rework` windows with the people on each. A story appears once however many people touch it, and its `Waits for` names its own dependencies where it has them and its epic's otherwise. **A story's bar is the window it is open, not the time it is worked** — a story waiting between its build and its review sits inside its own bar, the header says so, and the component rows underneath are where the real windows are. The chart carries no per-person rows; utilisation and idle hours are on the `Options` tab and each person's arrival week is in `plan.md`.
