...
| Mode | For | Runs |
| --- | --- | --- |
| `quick` | A go/no-go in minutes: is this a 400-hour deal or a 4,000-hour one? | Totals, splits and risk quadrant. Skips dependencies, span and sensitivity — which also makes it the one mode `/est-plan` cannot read, since a plan needs the dependency graph; renders `--formats md` only and records no ledger entry — a go/no-go is not an estimate anyone should later reconcile against |
...
- **The story sheets carry only the roles that work on stories.** QA is priced as a share of the story total and the architect as setup plus a capped weekly rate on the calendar, so neither sits on a story and neither gets `{role}_hours` columns on Stories or `alloc_` columns on Tasks — they would be zero on every row. Their hours are on their own `[project] qa` and `[project] architect` lines and in the role table, and the sheet says so in that row's description. The rule is `by_role[role].on_stories > 0`, not a list of names: a model that moves QA onto stories gets the columns back with no code change.
...
- **This skill does not answer how long it will take.** It used to print a span derived from `hours / six people`, and clients quoted it back as a schedule. The span is still computed, because the architect and ceremony lines are priced against it, and its basis is still stated as the premise of those numbers — but it is no longer presented as an answer. **`/est-plan` is where duration comes from**: it schedules the work over the dependency graph, staffs it, and re-prices every option against the calendar that option implies.
- **And it never becomes a date.** Nothing downstream of this skill produces one. `/est-plan` reports relative weeks, `W1..Wn`, because week 1 is whenever the project starts and turning that into a calendar is the client's decision and the client's risk.
