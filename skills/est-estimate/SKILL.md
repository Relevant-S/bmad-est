...
1. **Read the granularity report before you read the bands, and read it as a question about your tagging.** The checker's `granularity` block compares this inventory's surfaces per story against the anchor's 2.29, and `sizing.ceiling` reports whether stories are piled at the top band in use with the scale above it empty. Neither is a correction factor any more: the nine-band scale reaches from 1 to 34 complexity points and holds the same scope inside 15% across a 5x change of grain. What they tell you is which part of the scale this inventory needs. Rows written finer than the anchor belong in `XS` and `S` and are over-priced if they land on `M` by default; rows written coarser belong in `XXL` and above, and tagging them `XL` prices a row holding several stories as one. Settle that before you tag anything, and record in the extraction report if the grain is deliberate.
...
- **Risk is attributed to whoever carries it — per role, and per row.** A budget needs hours
  by role at the row level, because rates differ up to 3x between an architect and a QA; a
  buffer nobody can attribute to a role cannot be budgeted, since a role-hour has no price until
  you know whose hour it is. `{role}_risk` is that row's share of `uncertainty.model_risk`, split
  LINEARLY because systematic error is correlated by definition — if the size-band calibration is
  off it is off for every role and every row at once. So the column ADDS UP: sum it down the
  sheet and `by_role[role].model_risk_hours` comes back, and `confidence.role_attribution` states
  that reconciliation in numbers.
- **`planned` is the mean plus ONE sigma of systematic risk, and nothing else.** No feature
  variance, no widening for a thin brief. It is narrower than the role's own band and it is not a
  worst case — quote it as a budget line, never as "the high end". The BAND is the other quantity
  and it is NOT additive: variances combine in quadrature, so the role half-bands sum to about
  1.1x the project's on the anchor. Quote a role's own band; never add bands up. No per-row band
  column is written anywhere, precisely so there is nothing of that kind to sum.
- **Task figures are ALLOCATED, not estimated.** The cost model prices stories; a task is a
  source row kept so a reviewer can check the story against the client's own sentence, and no
  delivered project ever recorded what a task cost. The rule — an even split, named in
  `estimate.json`'s `task_allocation` — is the one that asserts least, and the columns are
  prefixed `alloc_` so nobody negotiates line by line over a number with no evidence behind it.
  They sum back to the parent story exactly, per role, at the precision they are published in.
...
- **Standing work is added, and said out loud.** The estimate prices setup, pipeline, environment and release work from the cost model's `standing_work` catalogue, because every project pays it and no client document describes it. The catalogue and its hours live in the model so every estimate prices the same pipeline at the same rate; **which items apply is the inventory's call**, recorded in its `standing_scope` at extraction time, and the `standing_work.selection` block reports what was claimed, what was declined and why. An item the inventory never mentions is priced anyway — the failure mode is paying twice, never silently dropping. It appears as its own lines in the feature table and its own `standing_work` block, never folded into the total silently. `--no-standing-work` drops the lot — only when the client is bringing a platform that already has all of it.
