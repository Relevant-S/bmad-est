...
class TestMarkdown(unittest.TestCase):
    ...
    def test_every_relation_on_a_role_row_is_checkable_by_the_reader(self):
        """`On stories` and `Project-level` are PERT means and sum to `Hours`, which is NOT the
        `Likely` beside it — that is the mode. Before `Hours` was on the row a reader could add
        the two sources up and match no column at all, and putting `Planned` next to `Likely`
        would have made that gap look like broken arithmetic instead of two honest figures."""
        md = render.markdown(estimate_for())
        for line in md.splitlines():
            if not line.startswith("| ") or "---" in line or line.startswith("| Role"):
                continue
            cells = [c.strip() for c in line.split("|")[1:-1]]
            if len(cells) != 9:
                continue
            num = lambda c: 0.0 if c == "—" else float(c.replace(",", ""))
            _, _, _, _, hours, on_stories, project, risk, planned = cells
            with self.subTest(role=cells[0]):
                self.assertAlmostEqual(num(hours), num(on_stories) + num(project), delta=1.0)
                self.assertAlmostEqual(num(planned), num(hours) + num(risk), delta=1.0)
...
class TheBudgetColumns(unittest.TestCase):
    """The sheets carry hours per role per row, with each row's own share of the buffer.

    A budget is impossible without this: rates differ up to 3x between an architect and a QA,
    so a project total in hours cannot be turned into money. The column a reader actually sums
    is `{role}_risk`, and the property that makes summing it legitimate is that the split is
    linear — so these tests sum it, rather than checking a cell in isolation.
    """

    def setUp(self):
        self.tmp = tempfile.TemporaryDirectory()
        self.dir = Path(self.tmp.name)
        self.inv = inventory([feature("F1", "Login", size="L"),
                              feature("F2", "Search", size="M")])
        self.inv["features"][0]["tasks"] = [
            {"id": "F1-T1", "name": "Form",
             "citations": [{"source_id": "S1", "location": "§1", "quote": "A login form."}]},
            {"id": "F1-T2", "name": "Session",
             "citations": [{"source_id": "S1", "location": "§2", "quote": "A session."}]}]
        self.est = est.build_estimate(self.inv, model(), options(granularity="project"))
        self.roles = render.role_names(self.est)
    ...
    def test_the_range_columns_that_were_here_first_did_not_move(self):
        """Other readers name `{role}_low|likely|high`. The new block is appended after them,
        never in place of them."""
        row = self.stories()[0]
        for role in self.roles:
            for suffix in ("low", "likely", "high"):
                self.assertIn(f"{role}_{suffix}", row)

    def test_every_role_carries_hours_risk_and_planned_on_every_row(self):
        for row in self.stories():
            for role in self.roles:
                for suffix in ("hours", "risk", "planned"):
                    self.assertIn(f"{role}_{suffix}", row, f"{role}_{suffix} missing")
        self.assertIn("risk_hours", self.stories()[0])
        self.assertIn("planned_hours", self.stories()[0])

    def test_summing_a_risk_column_down_the_sheet_reproduces_the_project_figure(self):
        """The whole point of the linear split. The sheet includes the `[project] …` rows, so
        the column covers the planning, QA and overhead that touch no story — which is where
        27% of a real project lives."""
        rows = self.stories()
        for role in self.roles:
            with self.subTest(role=role):
                total = sum(float(r[f"{role}_risk"] or 0) for r in rows)
                self.assertAlmostEqual(total, self.est["by_role"][role]["model_risk_hours"],
                                       delta=0.05 * len(rows) + 0.01)

    def test_a_row_planned_is_its_own_hours_plus_its_own_risk(self):
        """Never a project total repeated on a story line — that reading is what makes a sheet
        look broken, and it is answered by the arithmetic being checkable in the row."""
        for row in self.stories():
            for role in self.roles:
                with self.subTest(row=row["id"], role=role):
                    self.assertAlmostEqual(float(row[f"{role}_planned"] or 0),
                                           float(row[f"{role}_hours"] or 0)
                                           + float(row[f"{role}_risk"] or 0), delta=0.02)

    def test_the_project_rows_carry_their_own_risk_too(self):
        project = [r for r in self.stories() if r["origin"] == "project"]
        self.assertTrue(project)
        for row in project:
            self.assertGreater(float(row["risk_hours"] or 0), 0)

    def test_no_row_carries_a_band_column_anybody_could_sum(self):
        """Bands combine in quadrature. A per-row band column summed down the sheet reconciles
        with nothing, so none is written."""
        header = self.stories()[0]
        for role in self.roles:
            self.assertNotIn(f"{role}_band", header)
            self.assertNotIn(f"{role}_half_width", header)

    def test_the_task_columns_say_allocated_in_their_own_names(self):
        """The label is in the header on purpose. `dev_hours` on a task row would invite a
        negotiation over a figure no delivered project ever recorded; `alloc_dev_hours`
        cannot."""
        row = self.tasks()[0]
        for role in self.roles:
            for suffix in ("hours", "risk", "planned"):
                self.assertIn(f"alloc_{role}_{suffix}", row)
                self.assertNotIn(f"{role}_{suffix}", row)
        self.assertIn("alloc_hours", row)
        self.assertIn("alloc_planned_hours", row)

    def test_the_task_rows_sum_to_their_parent_story(self):
        rows = [r for r in self.tasks() if r["feature_id"] == "F1"]
        self.assertEqual(len(rows), 2)
        story = next(r for r in self.stories() if r["id"] == "F1")
        for role in self.roles:
            for suffix in ("hours", "risk", "planned"):
                with self.subTest(role=role, suffix=suffix):
                    self.assertAlmostEqual(
                        sum(float(r[f"alloc_{role}_{suffix}"] or 0) for r in rows),
                        float(story[f"{role}_{suffix}"] or 0), places=6,
                        msg="a reader adding this column up must land on the story's own figure")
        self.assertAlmostEqual(sum(float(r["alloc_hours"] or 0) for r in rows),
                               float(story["hours"]), places=6)

    def test_a_story_with_no_tasks_contributes_no_task_rows(self):
        self.assertEqual({r["feature_id"] for r in self.tasks()}, {"F1"})

    def test_the_markdown_role_table_carries_the_budget_pair_and_labels_it(self):
        md = render.markdown(self.est)
        self.assertIn("| Risk | Planned |", md)
        self.assertIn("ONE standard deviation of that risk and nothing else", md)
        self.assertIn("not a worst case", md)
        self.assertIn("the column ADDS UP", md)

    def test_the_brief_answers_how_many_hours_of_a_role_are_planned(self):
        """est-agent-estimator is asked this by name and reads the brief, not estimate.json."""
        brief = render.brief(self.est)
        for role, row in brief["by_role"].items():
            self.assertIn("risk_adjusted_hours", row, role)
        self.assertIsNotNone(brief["features"][0]["risk_adjusted_hours"])
