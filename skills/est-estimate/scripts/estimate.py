...
SPAN_KEYS = ("weeks", "weeks_range", "weeks_three_point", "assumed_team_size", "basis")


def check_span(span):
    """A span handed in from outside has to carry everything `duration()` returns.
    ...
    if missing:
        raise ValueError(
            f"the span handed to project_components is missing {', '.join(missing)}. It must "
            f"carry everything duration() returns — see est-plan/scripts/schedule.py, which is "
            f"the only thing that should be building one"
        )
    if len(span["weeks_three_point"]) != 3 or any(w < 0 for w in span["weeks_three_point"]):
        raise ValueError("weeks_three_point must be three non-negative weeks, low to high")
    return span
...
def project_components(priced, model, granularity, options):
    ...
    span = (check_span(dict(options["span"], critical_path_hours=path["hours"]))
            if options.get("span") else
            duration(priced, components, model, path, options["team_size"]))
