...
**Classify every story** on the five axes `est-estimate` consumes — size band, compressibility, review tier, clarity, novelty — plus `surfaces`, using `references/classification-guide.md`. Read the bands from the model that will price them first — `uv run scripts/inventory-check.py --bands --cost-model {project-root}/_bmad/memory/est/cost-model.json` prints the hour ranges, the worked exemplars for each band and the delivered project they were fitted against. The band is the tag that moves the number most and it is the one most often read as importance rather than size: centrality and risk are `review_tier`'s job, and sizing for them too bills the same fact twice. Each tag needs a one-line `why`, and the `why` has to be about *this* story: `inventory-check.py` reports a justification repeated across a fifth of the inventory, because a sentence reused two hundred times is a default wearing a reason. Capture dependencies while the source is still in front of you, because the sentence that implies one ("once login is in place, users can…") is gone by the time the estimate runs.
...
**Check.** `uv run scripts/inventory-check.py <workspace>/feature-inventory.json --normalized <workspace>/normalized --manifest <workspace>/normalized/manifest.json --cost-model {project-root}/_bmad/memory/est/cost-model.json` verifies every quote against the document it cites, resolves structured locations against anchors the file really has, reconciles converted sources against `sources`, and checks the schema, references and dependency graph. Its `unreferenced_regions` is the coverage pass done by search rather than memory. Fix every finding, judge each unreferenced region for substance, re-run until clean. Its `sizing` block compares the band distribution against the delivery anchor and does not block: a band shape away from the anchor can be right, but it has to be a decision — answer each sizing warning in the extraction report, or re-tag.
